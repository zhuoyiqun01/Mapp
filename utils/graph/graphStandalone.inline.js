(() => {
  ...
  function applyGraphWeightedCircleLayout(cy, layers) {
    ...
    for (const key of keysOrdered) {
      ...
      sorted.push(...groupNodes);
    }
    const visible = sorted.filter((node) => !hiddenSet.has(String(node.data("tagGroup") ?? "").trim()));
    const hiddenNodes = sorted.filter((node) => hiddenSet.has(String(node.data("tagGroup") ?? "").trim()));
    const w = cy.width();
    const h = cy.height();
    const cx = w / 2;
    const cyy = h / 2;
    ...
    visible.forEach((node, i) => {
      const angle = 2 * Math.PI * i / Math.max(1, nv) - Math.PI / 2;
  ...
  function applyGraphTimeLayout(cy, alertFn, layoutOpts, graphLayers = null) {
    ...
    const hiddenSet = /* @__PURE__ */ new Set((graphLayers?.hidden ?? []).map((h) => String(h).trim()));
    const keysOrdered = graphLayers ? orderedTagGroupKeysFromState(tagKeysFromNodes, graphLayers) : [...tagKeysFromNodes].sort((a, b) => {
      if (a === "" && b !== "") return 1;
      if (b === "" && a !== "") return -1;
      return a.localeCompare(b, GRAPH_SORT_LOCALE);
    });
  ...
  function applyGraphNeighborHighlight(cy, centerId) {
    cy.batch(() => {
      ...
      el.connectedEdges().forEach((edge) => {
        edge.addClass("focus-e");
        edge.connectedNodes().forEach((node) => {
          if (node.id() !== centerId) node.addClass("focus-nh");
        });
      });
    });
  }
  ...
  function wireStandaloneGraphControls(cy, payload, safeName) {
    ...
    if (time) {
      time.onclick = () => applyGraphTimeLayout(
        cy,
        undefined,
        { weightBias: payload.graphTimeAxisWeightBias ?? 0 },
        payload.graphLayers ?? null
      );
    }
